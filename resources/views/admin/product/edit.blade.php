    ...
    <form class="form" action="/admin/product/update/{{$data->id}}" method="post" enctype="multipart/form-data">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <select class="form-control select2 select2-hidden-accessible" name="collection_id" style="width: 100%;">
            @foreach($collections as $rs)
            <option value="{{$rs->id}}" @if($rs->id == $data->collection_id) selected="selected" @endif>{{$rs->title}}</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
